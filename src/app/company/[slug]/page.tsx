...
function CompanyDetail({ params }: CompanyPageProps) {
  ...
  useEffect(() => {
    ...
    // Fetch this company's salaries from the secure backend search API
    fetch(`/api/compensation/search?company=${encodeURIComponent(slug)}`)
      .then((res) => res.json())
      .then((data) => {
        if (data.results) {
  ...
  const chartData = salariesList.map((r) => {
  ...
  // Compute metrics
  const avgSalary = chartData.length > 0 ? Math.round(
    chartData.reduce((sum, r) => sum + r.Total, 0) / chartData.length
  ) : 0;
  const maxSalary = chartData.length > 0 ? Math.max(...chartData.map((r) => r.Total)) : 0;
  ...
  return (
    <div className="min-h-screen flex flex-col justify-between" style={{ background: "var(--background)" }}>
      ...
      <div style={{ display: "flex", justifyContent: "center", width: "100%", flex: 1 }}>
        ...
        {loading ? (
          <div className="glass-card p-12 text-center text-xs w-full flex items-center justify-center min-h-[300px]" style={{ color: "var(--muted)" }}>
            <div className="flex flex-col items-center justify-center gap-2">
              <div className="w-6 h-6 border-2 border-cyan-400 border-t-transparent rounded-full animate-spin"></div>
              <span>Verifying session and retrieving salary leveling benchmarks for {companyName}...</span>
            </div>
          </div>
        ) : (
          <>
